...
int main(void)
{
    ...
    for(uint32_t i = 0; i < nItem2 ; i++ )
    {
    	...
    	// can do this also since scanf expects an address
    	// array is already an address, the base address, so no need to do &
    	scanf("%d",(array2+i));
    }
...
void swap_arrays(int32_t *array1, int32_t *array2, uint32_t nitem1, uint32_t nitem2)
{
   // if nitem1 < nitem2, then len = nitem1, if not then len = nitem2
   // basically just finding which array is the shortest
