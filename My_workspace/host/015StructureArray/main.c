...
void display_all_records(STUDENT_INFO_t *record, int8_t max_record)
{
    ...
    {
    	// to check if the rollNumber is zero or not, if its zero then false
    	// got condition to make sure it is not zero
...
int delete_record(STUDENT_INFO_t *record, int8_t max_record)
{
     ...
     for(int i = 0 ; i < max_record ; i++)
    {
        ...
        {
            delete_flag = 1;
            //memset is to set a block of memory to a specific value given the memory address
            //here is setting the &record[i to 0, with number of sizeof(STUDENT_INFO_t) bytes
